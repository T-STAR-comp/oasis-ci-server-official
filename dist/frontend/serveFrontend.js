...
function assertSsrBundleResolvable(serverEntry) {
    ...
    if (!fs.existsSync(ssrModules) && !fs.existsSync(rootModules)) {
        throw new Error("SSR bundle requires h3-v2. Run `npm install` in oasis-ci-server, or redeploy with `npm run build:deploy` (installs app-dist/server/node_modules).");
    }
}
