...
authRouter.post("/api/auth/sign-in", validate(z.object({ email: z.string().email(), password: z.string().min(1) })), async (req, res) => {
    ...
    if (!user)
        return sendFail(res, "No account matches that email.", 404);
    ...
    if (!passwordIsValid) {
        if (!user.passwordHash) {
            return sendFail(res, "This account has no password set yet. Set BOOTSTRAP_ADMIN_PASSWORD and sign in with it once.", 401);
        }
    ...
    if (user.status === "pending_review" || user.status === "suspended") {
        return sendFail(res, user.status === "pending_review"
...
authRouter.post("/api/auth/profile", withSession(true), validate(z.object({
    ...
    if (!req.user)
        return sendFail(res, "You must be signed in.", 401);
    const state = await readState();
