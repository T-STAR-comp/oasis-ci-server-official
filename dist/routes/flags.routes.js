...
flagsRouter.post("/api/flags", withSession(true), requireRole(["owner", "pen_tester", "moderator", "admin"]), validate(z.object({
    ...
    state.auditLog = [
        createAuditEvent(req.user.name, "Flag filed", exposure.id, `${title} on ${exposure.domain}`),
        ...state.auditLog,
    ];
...
flagsRouter.post("/api/flags/resolve", withSession(true), requireRole(["moderator", "admin"]), validate(z.object({ flagId: z.string().min(1), status: z.enum(["open", "resolved", "dismissed"]) })), async (req, res) => {
    const state = await readState();
    const flag = state.flags.find((item) => item.id === req.body.flagId);
    if (!flag)
        return sendFail(res, "The selected flag was not found.", 404);
    state.flags = state.flags.map((item) => item.id === flag.id ? { ...item, status: req.body.status } : item);
