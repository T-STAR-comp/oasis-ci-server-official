...
submissionsRouter.post("/api/submissions", withSession(true), requireRole(["pen_tester", "moderator", "admin"]), validate(z.object({
    fullUrl: z
        .string()
        .min(1)
        .transform((value) => {
    ...
    const domainRecord = state.domains.find((entry) => entry.domain === domain) ?? buildClaimlessDomain(domain);
...
submissionsRouter.post("/api/submissions/review", withSession(true), requireRole(["moderator", "admin"]), validate(z.object({
    ...
    if (!submission)
        return sendFail(res, "The submission could not be found.", 404);
    const nextStatus = req.body.decision === "approve" ? "approved" : "rejected";
    state.submissions = state.submissions.map((item) => item.id === submission.id ? { ...item, status: nextStatus, moderatorNote: req.body.moderatorNote } : item);
    state.exposures = state.exposures.map((item) => item.id === submission.exposureId
        ? {
            ...item,
