...
usersRouter.post("/api/users", withSession(true), requireRole(["moderator", "admin"]), validate(z.object({
    ...
    if (!user)
        return sendFail(res, "That user could not be found.", 404);
    ...
    if (req.user.role === "moderator") {
        const allowed = user.role === "pen_tester" &&
            req.body.role == null &&
            ["active", "suspended"].includes(req.body.status);
        if (!allowed)
            return sendFail(res, "Moderators can only verify or suspend pen tester accounts.", 403);
    }
    const passwordHash = req.body.password ? await hashPassword(req.body.password) : undefined;
    state.users = state.users.map((item) => item.id === user.id
        ? {
            ...item,
...
usersRouter.post("/api/researcher-accounts", validate(z.object({
    ...
    sendFail(res, "Pen tester account requests are not available. Owners may contact Oasis CI for remediation assistance using the address configured by your administrator.", 403);
});
usersRouter.post("/api/users/delete", withSession(true), requireRole(["admin"]), validate(z.object({ userId: z.string().min(1) })), async (req, res) => {
    const state = await readState();
    const user = state.users.find((item) => item.id === req.body.userId);
    if (!user)
        return sendFail(res, "That user could not be found.", 404);
    if (user.id === req.user.id)
        return sendFail(res, "Admins cannot delete their own signed-in account.", 400);
    state.domains = state.domains.map((domain) => domain.ownerUserId === user.id
        ? { ...domain, ownerUserId: undefined, verificationStatus: "unclaimed", ownerAccessExpiresAt: undefined }
        : domain);
    state.exposures = state.exposures.map((exposure) => exposure.submittedBy === user.id ? { ...exposure, submittedBy: undefined } : exposure);
...
usersRouter.post("/api/moderators", withSession(true), requireRole(["admin"]), validate(z.object({
    name: z.string().min(1).max(120),
    email: z.string().email(),
    title: z.string().max(160).optional(),
    password: z.string().min(8).max(128).optional(),
