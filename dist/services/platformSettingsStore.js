...
function withPolicyMeta(settings) {
    return {
...
export async function ensurePlatformSettingsTable() {
  ...
  `);
    ...
    if (Array.isArray(rows) && rows.length === 0) {
        await db.query(`INSERT INTO platform_settings (id, remediation_email, remediation_phone, updated_at)
...
export async function updatePlatformSettings(input) {
    ...
    await db.query(`UPDATE platform_settings SET remediation_email = :email, remediation_phone = :phone, updated_at = NOW() WHERE id = 1`, { email, phone });
