...
export async function ensureSessionsTable() {
  ...
  `);
    const [fkRows] = await db.query(`SELECT CONSTRAINT_NAME AS name
...
export async function readSessionByCsrfToken(csrfToken) {
    await ensureSessionsTable();
    const db = requirePool();
    const [rows] = await db.query(`SELECT id, user_id, csrf_token, created_at
     FROM sessions
     WHERE csrf_token = :csrf_token AND expires_at > UTC_TIMESTAMP()
     LIMIT 1`, { csrf_token: csrfToken });
    const row = rows[0];
    if (!row)
        return null;
...
export async function touchSession(sessionId) {
    ...
    await db.query(`UPDATE sessions
     ...
     WHERE id = :id`, { id: sessionId, ttl_hours: sessionTtlHours() });
}
