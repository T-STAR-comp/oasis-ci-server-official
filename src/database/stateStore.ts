...
export async function initDatabase() {
  ...
  if (!mysqlReady) {
    throw new Error(
      "Missing MySQL configuration. Set MYSQL_HOST, MYSQL_USER, MYSQL_PASSWORD, MYSQL_DATABASE (and optionally MYSQL_PORT).",
    );
  }
...
async function ensureSchemaMigrations() {
  ...
  for (const column of userPolicyColumns) {
    const [rows] = await db.query(
      ...
      { database: env.mysql.database, column: column.name },
    );
  ...
  const [recommendationColumn] = await db.query(
    `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = :database AND TABLE_NAME = 'exposures' AND COLUMN_NAME = 'remediation_recommendation'`,
    { database: env.mysql.database },
  );
  if (Array.isArray(recommendationColumn) && recommendationColumn.length === 0) {
    await db.query(
      "ALTER TABLE exposures ADD COLUMN remediation_recommendation TEXT NOT NULL DEFAULT '' AFTER internal_note",
    );
  }

  const exposureCategoryEnum =
    "ENUM('sensitive_data','open_directory','admin_panel','backup_config','domain_extension','impersonation_risk','expired_ssl')";
  for (const table of ["exposures", "submissions", "flags"] as const) {
    const [categoryColumn] = await db.query(
      `SELECT COLUMN_TYPE AS column_type
       FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = :database AND TABLE_NAME = :table AND COLUMN_NAME = 'category'`,
      { database: env.mysql.database, table },
    );
    const columnType = (categoryColumn as { column_type?: string }[])[0]?.column_type ?? "";
  ...
  for (const column of flagColumns) {
    const [rows] = await db.query(
      `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = :database AND TABLE_NAME = 'flags' AND COLUMN_NAME = :column`,
      { database: env.mysql.database, column },
    );
    if (Array.isArray(rows) && rows.length === 0) {
      if (column === "domain") {
        ...
        await db.query(
          "ALTER TABLE flags ADD COLUMN category ENUM('sensitive_data','open_directory','admin_panel','backup_config','domain_extension','impersonation_risk','expired_ssl') NOT NULL DEFAULT 'open_directory' AFTER exposure_title",
        );
      } else if (column === "severity") {
        await db.query(
          "ALTER TABLE flags ADD COLUMN severity ENUM('critical','high','medium','low','info') NOT NULL DEFAULT 'medium' AFTER category",
        );
      } else if (column === "exposure_listing_status") {
        await db.query(
          "ALTER TABLE flags ADD COLUMN exposure_listing_status ENUM('approved','pending_review','rejected','fixed','archived') NOT NULL DEFAULT 'approved' AFTER severity",
        );
      } else if (column === "exposure_remediation_status") {
        await db.query(
          "ALTER TABLE flags ADD COLUMN exposure_remediation_status ENUM('not_started','in_progress','fixed') NOT NULL DEFAULT 'not_started' AFTER exposure_listing_status",
        );
        ...
        await db.query(
          "ALTER TABLE flags ADD COLUMN reporter_role ENUM('owner','pen_tester','moderator','admin') NOT NULL DEFAULT 'owner' AFTER reporter_name",
        );
      } else if (column === "flag_type") {
        await db.query(
          "ALTER TABLE flags ADD COLUMN flag_type ENUM('false_positive','review_request','escalation') NOT NULL DEFAULT 'review_request' AFTER reporter_role",
        );
...
export async function readState(): Promise<PlatformState> {
  const db = requirePool();

  const [userRows] = await db.query("SELECT * FROM users ORDER BY created_at DESC");
  const users: User[] = (userRows as any[]).map((row) => ({
    id: row.id,
    name: row.name,
    email: row.email,
    role: row.role,
  ...
  const domains: DomainRecord[] = (domainRows as any[]).map((row) => ({
    id: row.id,
    domain: row.domain,
  ...
  const historyByExposure = new Map<string, ExposureHistoryEvent[]>();
  for (const row of historyRows as any[]) {
    const item: ExposureHistoryEvent = {
      id: row.id,
  ...
  const exposures: Exposure[] = (exposureRows as any[]).map((row) => ({
    id: row.id,
    domain: row.domain,
  ...
  const submissions: Submission[] = (submissionRows as any[]).map((row) => ({
    id: row.id,
  ...
  const flags: ReviewFlag[] = (flagRows as any[]).map((row) => ({
    id: row.id,
    exposureId: row.exposure_id,
  ...
  const [notificationRows] = await db.query(
    "SELECT * FROM user_notifications ORDER BY created_at DESC LIMIT 500",
  );
  const notifications: UserNotification[] = (notificationRows as any[]).map((row) => ({
    id: row.id,
    userId: row.user_id,
    type: row.type,
    title: row.title,
  ...
  const claims: ClaimRequest[] = (claimRows as any[]).map((row) => ({
    id: row.id,
    domain: row.domain,
    method: row.method,
    contact: row.contact,
  ...
  const auditLog: AuditEvent[] = (auditRows as any[]).map((row) => ({
    id: row.id,
    actor: row.actor,
    action: row.action,
    target: row.target,
...
export async function writeState(state: PlatformState) {
  const db = requirePool();
  ...
  try {
    ...
    for (const user of state.users) {
      await connection.query(
        `INSERT INTO users (id, name, email, role, status, title, company, verified_domains, password_hint, password_hash, policies_accepted_version, policies_accepted_at)
    ...
    for (const audit of state.auditLog) {
      await connection.query(
        `INSERT INTO audit_events (id, actor, action, target, detail, created_at)
...
function buildAnalytics(exposures: Exposure[], submissions: Submission[]) {
