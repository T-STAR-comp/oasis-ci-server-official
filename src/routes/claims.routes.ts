...
import {
  ...
  isDomainClaimedByAnother,
  upsertVerifiedDomain,
  ...
  async (req, res) => {
    try {
      ...
      if (!requiredEmail) {
        ...
        return sendFail(
          res,
          "No admin contact email is configured for this exposure/company profile. Ask an admin to set it first.",
          400,
        );
      }
      ...
      if (submittedContact !== requiredEmail) {
        ...
        return sendFail(
          res,
          `Verification codes are only sent to the registered contact: ${requiredEmail}`,
          400,
        );
      }
      const claim = {
      ...
      state.auditLog = [
        createAuditEvent(
          req.user?.name ?? "Visitor",
          "Ownership claim started",
          domain,
          `Verification token prepared via ${claim.method}.`,
        ),
        ...state.auditLog,
      ];
...
claimsRouter.post(
  ...
  async (req, res) => {
    ...
    if (!claim) return sendFail(res, "That claim request was not found.", 404);
    ...
    const ownerTarget =
      req.user && ["owner", "moderator", "admin"].includes(req.user.role)
        ? req.user.id
        : createId("owner");
    ...
    if (!ownerUserExists) {
      const ownerAccount: User = recordPolicyAcceptance({
        id: ownerTarget,
        name: ownerName,
        email: ownerEmail,
        role: "owner",
    ...
    state.claims = state.claims.map((item) =>
    ...
    sendOk(
      res,
      { user: state.users.find((user) => user.id === ownerTarget) ?? null, csrfToken },
      `Ownership verified for ${claim.domain}.`,
    );
  },
);
