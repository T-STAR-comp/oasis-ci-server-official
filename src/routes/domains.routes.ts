...
domainsRouter.post(
  ...
  validate(z.object({ domain: z.string().min(1).max(255) })),
  async (req, res) => {
    const state = await readState();
    const domain = parseDomain(req.body.domain);
    const domainExposures = state.exposures.filter((item) => item.domain === domain);
    if (domainExposures.length === 0) return sendFail(res, "No exposures were found for that domain.", 404);
    if (!domainExposures.every((item) => item.removalReviewStatus === "verified_removed")) {
      return sendFail(
        res,
        "Each flaw must be verified by a moderator before the domain can be removed from the directory.",
        400,
      );
    }
