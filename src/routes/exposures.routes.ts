...
import {
  sendExposureNotificationEmail,
  sendOwnerFixDeniedEmail,
  sendOwnerFixVerifiedEmail,
...
exposuresRouter.post(
  "/api/exposures",
  withSession(true),
  requireRole(["owner", "moderator", "admin"]),
  validate(exposureSchema),
  async (req, res) => {
    ...
    if (!exposure) return sendFail(res, "The selected exposure could not be found.", 404);
    ...
    if (req.user?.role === "owner" && exposure.removalReviewStatus === "verified_removed") {
      return sendFail(
        res,
        "This exposure was verified by a moderator. Contact Oasis CI to request a reversal before editing.",
        403,
      );
    }
    ...
    const ownerRequestedRemoval =
      req.user?.role === "owner" &&
      input.remediationStatus === "fixed" &&
      exposure.removalReviewStatus !== "verified_removed";
    const moderatorOverrideRemoval =
      (req.user?.role === "moderator" || req.user?.role === "admin") && input.removalReviewStatus != null;
    const updated: Exposure = {
      ...updatedBase,
      removalReviewStatus: ownerRequestedRemoval
        ? "requested"
        : moderatorOverrideRemoval
          ? (input.removalReviewStatus as any)
...
exposuresRouter.post(
  "/api/exposures/:id/verify-removal",
  withSession(true),
  requireRole(["moderator", "admin"]),
  async (req, res) => {
    const state = await readState();
    const exposure = state.exposures.find((item) => item.id === req.params.id);
    if (!exposure) return sendFail(res, "The requested exposure was not found.", 404);
    ...
    const allVerified = remaining.every((item) =>
      item.id === updated.id ? updated.removalReviewStatus === "verified_removed" : item.removalReviewStatus === "verified_removed",
    );
...
exposuresRouter.post(
  "/api/exposures/:id/deny-fix",
  withSession(true),
  requireRole(["moderator", "admin"]),
  validate(z.object({ moderatorNote: z.string().max(4000).optional() })),
  async (req, res) => {
    const state = await readState();
    const exposure = state.exposures.find((item) => item.id === req.params.id);
    if (!exposure) return sendFail(res, "The requested exposure was not found.", 404);
...
exposuresRouter.post(
  "/api/exposures/:id/reverse-verification",
  withSession(true),
  requireRole(["moderator", "admin"]),
  validate(z.object({ moderatorNote: z.string().max(4000).optional() })),
  async (req, res) => {
    ...
    const timestamp = new Date().toISOString();
    const note =
      req.body.moderatorNote?.trim() || "Moderator reversed verification so the owner can continue work.";
    const updated: Exposure = {
      ...exposure,
      status: "approved",
      remediationStatus: "in_progress",
...
async function editExposure(req: Request, res: Response) {
  const state = await readState();
  const input = req.body;
  const exposure = state.exposures.find((item) => item.id === input.exposureId);
  if (!exposure) return sendFail(res, "That exposure could not be found.", 404);
  ...
  const updated: Exposure = {
    ...
    remediationRecommendation:
      input.remediationRecommendation?.trim() ?? exposure.remediationRecommendation,
    history: [
      {
        id: createId("hist"),
        actor: req.user!.name,
...
async function createExposure(req: Request, res: Response) {
  const state = await readState();
  const input = req.body;
  ...
  const domainRecord: DomainRecord = existingDomain ?? {
  ...
  const exposure: Exposure = {
    ...
    category: input.category as ExposureCategory,
    ...
    status: input.status as ExposureStatus,
    remediationStatus: input.remediationStatus as RemediationStatus,
  ...
  if (!existingDomain) state.domains = [domainRecord, ...state.domains];
