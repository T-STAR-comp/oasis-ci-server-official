...
policiesRouter.post(
  ...
  async (req, res) => {
    if (!req.user) return sendFail(res, "You must be signed in to accept policies.", 401);
    if (req.body.version !== CURRENT_POLICIES_VERSION) {
      return sendFail(
        res,
        "These policies have been updated. Refresh the page and review the current version before accepting.",
        409,
      );
    }
    ...
    state.auditLog = [
      createAuditEvent(
        ...
        "Policies accepted",
        CURRENT_POLICIES_VERSION,
        "User agreed to Oasis CI platform policies.",
      ),
      ...state.auditLog,
    ];
    ...
    sendOk(
      res,
      { user: { ...updated, passwordHash: undefined }, csrfToken: req.sessionRecord?.csrfToken },
